...
export default function Header({ onQuoteClick }: HeaderProps) {
  ...
  // Keep active tab in sync with the route (mainly for /blogs vs /)
  useEffect(() => {
  ...
  const getDesktopNavClass = (key: NavKey) => {
    ...
    const inactive = ' text-gray-700 dark:text-gray-300 hover:text-emerald-600 dark:hover:text-white';
  ...
  const getMobileNavClass = (key: NavKey) => {
    const base = 'block w-full text-left px-2 py-2 rounded-lg transition-all duration-300 font-medium text-xl sm:text-lg';
